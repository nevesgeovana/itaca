...
\midrule
\code{set\_uncertainty}                 & 2.3.1, 4.2, 5.1.2  & Standard uncertainty $u(x_i)$ as Type B input. The declared magnitude must be finite (REQ-39): 5.1.2 defines the combined standard uncertainty as a positive square root, which excludes NaN, and as an estimated standard deviation characterizing a dispersion of attributable values, which excludes an infinity.\\
AST propagation engine                  & 5.1, 5.2    & Law of Propagation of Uncertainty (LPU) with first-order Taylor and chain rule on the AST.\\
...
Native serialization of provenance graphs to
PROV-N and PROV-JSON via \code{db.export\_provenance(path,
format="prov-json")} (or \code{format="prov-n"}) is an M2 deliverable
(v0.3.0; see Chapter~\ref{ch:roadmap}), enabling
interoperability with provenance-aware data catalogs without forcing
PROV serialization in every export path. The mapping above is the model
it will serialize; it is NOT implemented in v0.1.0 or v0.2.0. An earlier
revision of this chapter stated that v0.1.0 supported it, which was
false: no \code{export\_provenance} symbol exists in the released
package (\code{ITACA-012}).

\section{Code and Documentation: PEP 8, NumPy Docstring}

\itc{} code follows PEP~8~\citep{pep8}. Public-API docstrings follow the
NumPy docstring convention. Ruff's pydocstyle rules enforce PRESENCE
and STYLE; the \emph{Parameters} and \emph{Returns} sections on the
declared public surface are enforced by
\code{tests/test\_docstring\_sections.py} (REQ-79), not by linting.
REQ-79 names four sections and that checker pins two, so
\emph{Raises} and \emph{Examples} completeness is machine-checked by
nothing; an earlier revision of this paragraph claimed SECTION
COMPLETENESS without that qualifier, overclaiming in the same
direction as the defect it was correcting.
An earlier revision claimed linting enforced the sections,
which it does not: ruff checks that a docstring exists and is
well-formed, never that it carries Parameters or Examples
(\code{ITACA-016}).
