...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  \item \code{[uncertainties]}: values are float (absolute) or string
    ...
    from the declared value and no uncertainty the frame arrived with is
    ever read for a name the file declares. Does the FILE WRITE the name,
    as an equation target or a correction target? Then the declaration is
    applied again once the first line that writes it has run, overriding
    the value propagation gave it, so the frame does not ship a
    declaration as the propagation of itself (REQ-41, REQ-99).
    ...
    What remains open is a name written MORE than once, by an equation and
    then by a correction. The declaration is applied after the FIRST line
    that writes it, so a later line rewriting the same name propagates
    over it; whether that is the right reading is OQ-43.
