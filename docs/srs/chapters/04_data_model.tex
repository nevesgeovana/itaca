...
unchanged: the state hash covers the recovered data, while
\code{steps\_hash} covers the recipe. Without it an edited step passes
the state check, because the stored data still matches itself, and the
edited recipe then steers the next replay. Loading refuses a schema 2
archive whose \code{steps\_hash} is absent or does not match, and
refuses any archive with no state hash.
