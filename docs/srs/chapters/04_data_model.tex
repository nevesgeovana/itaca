...
are obtained symbolically via chain rule on the AST expression tree
(Section~\ref{sec:ast-engine}).
...
a defined scope, and REQ-103 defines it as a semantic guarantee: two
VarFrames in the same semantic state have the same hash, where same
semantic state means agreeing dtypes, shapes and bit patterns together
with agreeing metadata. The \code{state\_hash} of a VarFrame therefore
covers: dimension
names, coordinate arrays and dimension metadata; variable names, value
arrays and variable metadata; the ordered
operation sequence with normalized arguments and comments; the
uncertainty, correlation, and origin-tag arrays when present; and the
axis registry. Unit is covered even though
Section~\ref{sec:dimension-object} calls it metadata, because \code{rotate}
interprets it (REQ-101): a field an operation acts on is part of the
state, whatever it is called elsewhere. It excludes
...
\begin{itemize}[noitemsep,topsep=2pt]
  \item \code{[meta]}: required. All sub-fields are optional strings,
    with one typed exception: \code{idempotent} is a boolean declaring
    whether reapplying the workflow is meaningful (REQ-47). It is
    written unquoted; a quoted \code{"True"} is refused rather than
    silently ignored. Idempotence lives in the file because REQ-48 makes
    the file define the workflow in full, and idempotence decides
    whether that workflow may legally re-run (DD-36).
  ...
  \item \code{[uncertainties]}: values are float (absolute) or string
    ending in \code{"\%"} (relative). Per-variable. WHEN a declaration is
    applied is decided by two independent questions, and a name may answer
    yes to both.

    Does the VarFrame CARRY the name? Then the declaration is applied
    before the first line runs, so every line reading that name propagates
    from the declared SYSTEMATIC value rather than from whatever systematic
    component the frame arrived with. Does the FILE WRITE the name, as an
    equation target or a correction target? Then the declaration is applied
    again once the first line that writes it has run, overriding the value
    propagation gave it, so the frame does not ship a declaration as the
    propagation of itself (REQ-41, REQ-99).

    A declaration speaks for the SYSTEMATIC component alone, which is what
    \code{set\_uncertainty} assigns by default and what Chapter~8
    describes. A random component the frame arrived with is therefore kept
    and propagates untouched, so an \code{.itceq} file cannot presently
    specify the whole uncertainty of a name it declares. Whether it should
    be able to is OQ-45.

    The two are NOT exclusive, and the overlap is the case that decides
    the rule: a name the frame carries and the file also writes is
    assigned twice, once before and once after, because \code{[equations]}
    is evaluated before \code{[corrections]} and a line reading the name
    in between must see the declared value. They are jointly exhaustive,
    since \code{validate} refuses a declared name that is neither carried
    nor written (REQ-45), so every declaration is applied at least once
    and none is silently dropped.

    Both single-question rules have produced a wrong number rather than a
    missing one, which is why the rule is stated as two questions.
    Deciding only by what the frame carried made one file give two
    answers, a written name the frame happened to carry shipping the
    uncertainty its own equation had propagated in place of the declared
    one (\code{R4-ITA-003}). Deciding only by what the file writes
    mirrored it: a carried name that a correction rewrites was withheld
    from the equations that READ it, so a quantity derived from it
    propagated from the frame's own value, which the declaration
    overrides.

    Two consequences of the overlap are open questions rather than settled
    rules, and both are named here so the section is not read as deciding
    them. A name written MORE than once, by an equation and then by a
    correction, takes the declaration after the FIRST line that writes it,
    so a later line rewriting the same name propagates over it (OQ-43). And
    a RELATIVE declaration on a name assigned twice resolves twice, against
    the values the name holds at each moment, so one declaration yields two
    numbers and a line evaluated between the two propagates from the first
    (OQ-44). Both are reachable through legal files; neither is a rule this
    section asserts.
  ...
  \item \code{[corrections]}: evaluated after \code{[equations]}. May
    replace existing variables, whether the variable was produced by
    \code{[equations]} or supplied by the VarFrame; a correction that
    reads the name it writes reads the value that existed before it, and
    a variable a correction replaces but the file does not produce is a
    required input.
  \item A name declared in \code{[constants]} may not also be an
    equation or correction target. A constant is substituted into every
    read before evaluation, so such an equation would run and its result
    would never be read by anything in the file (DD-37). Redefinition
    within the equation sections is a different thing and stays legal:
    that is the replacement rule above. The same collision against a
    MEASURED channel, where the frame carries a variable of that name,
    is refused by \code{validate} rather than when the file is read,
    because only that step sees both the file and the frame. It is the
    more dangerous of the two: the equation runs on the declared number
    and the measurement is never read, so the result is wrong rather
    than merely unreachable (DD-39, OQ-31).
  \item In file order, an equation may only read what an earlier line
    produced. A forward reference is refused when the file is read,
    because the VarFrame may carry the same name, in which case the
    equation would silently use that value and the later line would
    overwrite it (REQ-48, DD-17).
\end{itemize}

\section{The .itc Native Binary Format}
\label{sec:itc-archive}
...
The current schema is \code{itaca-itc/3}, and it is the only one this
build reads. Its two predecessors are named here because a user will
meet them: \code{itaca-itc/1} is the 0.1.0 layout, and
\code{itaca-itc/2} added the replay step to each \code{history.json}
entry so a pipeline could be lifted from a reopened archive
(Section~\ref{sec:itc-pipe}). Both are REFUSED by name, with a message
saying what the archive lacks and instructing the reader to re-export
from the source data. Neither records a spatial coordinate system, so a
frame saved in polar coordinates cannot be reconstructed from one and
would come back cartesian, integrating against the wrong area element;
reading them would therefore reintroduce the defect this schema exists
to close (DD-47).

Schema 3 carries three digests in \code{metadata.json}, and they answer
three different questions.

\begin{itemize}[noitemsep,topsep=2pt]
  \item \code{state\_hash}, the REQ-103 state hash, covers the recovered
    state: the data together with the ordered operation sequence and its
    comments, the coordinate system and the operating mode.
  \item \code{steps\_hash} covers the structured replay arguments, which
    the state hash deliberately excludes. Without it an edited argument
    passes the state check, because the operation text and the stored
    data still match themselves, and the edited recipe then steers the
    next replay (DD-30). It is required unconditionally; the schema
    string no longer gates it, because a schema string is ordinary
    metadata and cannot gate an integrity check.
  \item \code{members}, the member manifest, maps every OTHER member to
    the SHA-256 of its bytes as written. \code{itc.open} verifies the
    member set in both directions and every digest before any other
    member is interpreted, so an edit that changes no recovered state at
    all, such as a forged per-entry history hash, is still refused.
\end{itemize}

The manifest cannot cover \code{metadata.json}, because that is where it
lives. This is a real gap and not a technicality: an edit to a field of
that member which no other check reads, such as \code{itaca\_version},
is not refused. The fields of it that matter each carry their own check,
the schema string against the readable set and the two digests against
what they recompute.

\textbf{What the archive guarantees is tamper EVIDENCE, not tamper
proofing.} A \code{.itc} carries no secret, so an editor who rewrites a
member and also recomputes \code{metadata.json} produces an archive that
opens. What the digests end is the case where an edit needs nothing else
at all, which is how one edited JSON string once turned a draft archive
into a production one and it reopened with a valid hash. An archive with
no state hash, no steps hash or no member manifest is refused.
...
\code{dims.json}         & Dimension names, coordinate arrays and their dtype, units, descriptions, numeric flags.\\
\code{vars\_meta.json}   & Variable units, descriptions, long names.\\
\code{frame.json}        & Frame-level state that is neither a dimension nor a variable: today the spatial coordinate-system tag (REQ-28).\\
...
\code{history.json}      & Ordered list of operation entries with indices, each carrying its replay step when the operation records one.\\
\code{metadata.json}     & \itc{} version, schema string, state hash, replay-steps hash, and the member manifest. The one member the manifest does not cover, since it is where the manifest lives.\\
\bottomrule
\end{tabularx}
\end{table}
