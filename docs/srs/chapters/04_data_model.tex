...
\code{db.set\_correlation(...)}. By default all correlations are zero and
...
and \code{-1} outside. \code{fill} sets \code{+1}. \code{fitvalue} sets
\code{+1} within the fit range and \code{-1} beyond it. The plotter consumes
...
as ``original'' as its weakest contributor).
...
A \code{.itc\_pipe} file is a human-readable serialization (TOML) of a
sequence of operations extracted from a VarFrame's History. It records:

\begin{itemize}[noitemsep,topsep=2pt]
  \item the \itc{} version that created it,
  \item the index range in the source history,
  \item each operation's name, positional and keyword arguments, and any
    attached comment,
