...
\code{history.json}      & Ordered list of operation entries with indices, each carrying its replay step when the operation records one (schema \code{itaca-itc/2}).\\
\code{metadata.json}     & \itc{} version, format version, state hash, and the replay-steps hash (schema \code{itaca-itc/2}).\\
\bottomrule
\end{tabularx}
\end{table}
