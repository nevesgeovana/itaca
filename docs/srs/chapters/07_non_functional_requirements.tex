...
upper bound}. Three rows are deliberately otherwise. \code{ruff} is an
EXACT pin (see below). \code{setuptools} and \code{setuptools-scm}
carry a floor with no ceiling, because they are build-time tools
resolved by the frontend in an isolated environment, where a ceiling
pins a consumer's toolchain rather than this project's.

CI runs the full test suite against the LATEST compatible versions on
every supported interpreter, and against the declared minimum of the
runtime dependencies \code{numpy} and \code{pandas}. It does NOT
exercise the floor of every row of the table; the development and
build-time floors are declared and not minimum-tested. That gap is
stated here rather than left to be inferred from the workflow, and it
is recorded as a known limitation rather than as a promise.
...
specifiers, and every row is checked against \code{pyproject.toml} by
...
deliberate. \code{ruff} is declared twice, here and as a
\code{pre-commit} hook revision that builds its own isolated
environment, so a range would let CI and the local hook resolve
different versions and run different rule sets. REQ-96 requires the two
to MIRROR each other and does not itself mandate an exact pin; the pin
is the means by which that mirror is kept, and
...
ONE DELIBERATE EXCEPTION to the mirror: the specification build REQ-95
requires is NOT mirrored here. It needs a TeX distribution, which is not a
contribution prerequisite, and a hook a contributor cannot run is a hook
that gets skipped. The exception is stated rather than left as a silent
gap between the two requirements.
\end{reqbox}
