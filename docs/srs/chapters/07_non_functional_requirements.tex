...
minimum tested version, the upper bound, and the role: where
\code{pyproject.toml} declares the dependency, or that it does not
declare it yet. CI runs the full test suite against the minimum and the
...
dependencies, stated a range for \code{ruff} where the file carries the
exact pin REQ-96 requires, and carried five rows for packages declared
in no extra at all, so a reader could not tell a policy from a plan.
...
environment, so a range would let CI and the local hook run different
rule sets (REQ-96). \code{scipy} and \code{uncertainties} are
