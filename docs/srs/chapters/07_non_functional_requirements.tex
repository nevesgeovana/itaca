...
\caption{Dependency version policy for \itc{}, as amended at the M1

Phase B3b checkpoint. Bounds are expressed as PEP 440 specifiers.}
...
This requirement does NOT enumerate them, and the absence is a

correction rather than an omission. It used to name three, which was

the count when the carve-out was written and was already false by the

time the release closed: the CHK-1 checkpoint turned five further

accepted inputs into public refusals, and nine breaking changes carry

the marker in \code{CHANGELOG.md}, measured on 2026-07-29. A second

inventory inside a normative document goes stale independently of the

one it duplicates, which is the defect this amendment closes rather

than repeats. The enumeration lives in \code{CHANGELOG.md}, which the

paragraph below already requires, and that file is the single authority

for what a release broke.
