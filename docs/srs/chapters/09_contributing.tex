...
Every \itc{} package imports only the Python standard library and
NumPy, except \code{io/} and \code{utils/}, which may import pandas
lazily (REQ-82 as amended, REQ-05, REQ-84). A \code{ruff}
...
test: add property-based test for AST sin/cos derivative identities
...
\begin{itemize}[leftmargin=2.5em,label=$\square$]
  ...
  \item No pandas/xarray/dask imports in any package except \code{io/}
        and \code{utils/} (REQ-82 as amended)
...
\begin{lstlisting}
polar = itc.processor("ft_drag_polar", config={"S_ref": 50.0})
polar.info()                         # prints: equations, uncertainties, required vars
polar.validate(db)                   # raises ProcessorValidationError if vars missing
                                     # or if a [constants] name is a measured channel
db_out = polar(db, comment="initial polar from FT-2026-A")
db_out = polar(db, report="reports/ft_drag.pdf")
\end{lstlisting}
