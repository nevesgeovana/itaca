...
nor enters the propagation. ONE class of expression is exempt and the
exemption is stated rather than implied: REQ-36 admits any
\code{np.*} function into an expression carrying no uncertainty, and a
reduction such as \code{np.max} reads cells other than its own, so an
expression containing one is evaluated over the whole grid as before.
Confining it would change the value of the reduction itself, which is a
different question from this requirement's.
\end{reqbox}
