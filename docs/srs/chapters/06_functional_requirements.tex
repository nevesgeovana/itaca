...
first rotation, so the recorded axis survives a save. The
surface uses ``axis'' for a coordinate system throughout, distinct
...
\code{validate} refuses two things: a variable the file needs that the
frame does not carry, and a name the file declares in \code{[constants]}
that the frame also carries as a measured variable. The second is
refused here rather than when the file is read, because the parser never
sees a frame, and the same file is legal against a campaign that does
not log that channel (DD-39, OQ-31). The call runs \code{validate}
first, so both refusals govern application as well.
\end{reqbox}
