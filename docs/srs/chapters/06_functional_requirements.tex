...
position, so every cell past that width was dropped with no signal while
Provenance went on hashing the complete file bytes: the source hash
certified content the frame did not represent. Such a row is refused with
\code{DataError}, naming the row, its field count, the header, and the
cells that would have been discarded (\code{R3-ITA-009}).

A row NARROWER than its header is NOT refused: its absent trailing cells
are filled with NaN, exactly as an empty cell is. The asymmetry is
deliberate, and it is stated here because a reader who meets one half of
it will otherwise infer the other wrongly. A row wider than its header is
a malformed file, either a truncated header or a delimiter inside an
unquoted field, and neither of those is repaired by discarding data; a
row narrower than its header is ordinary missing data, which this format
already represents as NaN everywhere else.
\end{reqbox}
...
wind-axis input came out claiming one axis for every row and the next
\code{db.rotate} transformed the rotated rows a second time
