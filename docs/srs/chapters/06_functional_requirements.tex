...
components, per REQ-98. \code{"polyfit"} produces no weights at all: it
evaluates the fitted polynomial directly, so there is nothing for the
propagation to consume, and filling with uncertainty present and
\code{method="polyfit"} raises \code{UncertaintyError} rather than
guessing a rule. It is therefore one of the five provisional operations
of REQ-98, pending OQ-18.
\end{reqbox}
...
than guessing, and each stays provisional until OQ-18 and OQ-24 are
resolved. The count is stated because it was previously carried in four
places at three different values while five operations refused, which
is the enumeration defect \code{ITC-20260729-1450} closes.

\code{fill(method="polyfit")} belongs to that family for a reason worth
recording, because a weaker one was written down first and is wrong. It
is NOT that the moving polynomial fit has data-dependent weights:
measured, its superposition departs by $1.3 \times 10^{-13}$, and applied
consistently that argument would draw \code{interpolate} into the family
as well. It is that the polyfit path has NO WEIGHT PATH AT ALL. The
\code{"linear"} and \code{"nearest"} methods emit a list of (neighbor,
weight) pairs that the propagation loop then consumes, while the polyfit
branch evaluates the fitted polynomial directly and emits no pairs, so
the loop has nothing to propagate through and skips those entries.
Propagating would mean inventing a weight rule the fit never produced,
which is precisely what OQ-18 has not yet frozen (REQ-26). The M0 rows
...
\code{fill} (\code{method="polyfit"}) & Same family and the same open
question (OQ-18): the polyfit path produces no weights for the
propagation to consume, so it raises when uncertainty is present. The
other two fill methods propagate, per their row above (REQ-26).\\
