...
\end{itemize}
Filled values are tagged \code{+1} in the HistoryFrame.
\end{reqbox}
...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  \item the constraint \code{window > deg} is enforced before any
    computation; violation raises \code{DiffWindowError};
...
the HistoryFrame; beyond the range, \code{-1}.
\end{reqbox}
...
interpolate, fill, fitvalue, and the reductions) are linear in the
variable values, so propagation through the operation weights is
exact; the Jacobian rows (\code{rotate}, \code{translate\_moments},
\code{combine}, \code{compute}) linearize a nonlinear map, and the
provisional rows raise until their rule is frozen. No operation
...
(\code{interpolate}, \code{fitvalue}, and the reductions) were
validated at the M1 Phase B1 checkpoint, with the mixed-sign
systematic behavior of the cubic and polyfit kernels carried as a
validation item alongside OQ-18.
...
unchanged.\\
\code{interpolate}, \code{fill}, \code{fitvalue} & Propagated through the
...
\code{fitmodel} & Coefficient-space rule not yet frozen (OQ-24);
raises when uncertainty is present until validated.\\
