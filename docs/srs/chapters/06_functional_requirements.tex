...
uncertainty, correlation and origin-tag arrays when present; and the
axis registry, comprising each registered frame's definition and
description and each vector group's components and source axis
(REQ-107). A metadata field left unset contributes nothing to the
digest, so a frame that declares none hashes exactly as it did before
metadata entered the scope.
