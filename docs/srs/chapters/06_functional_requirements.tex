...
member set in both directions and every digest BEFORE any member is
interpreted. What this delivers is tamper EVIDENCE and it must not be
read as more: a \code{.itc} carries no secret, so an editor who
rewrites a member and also recomputes the manifest produces an archive
that opens. What it ends is the case where an edit needs nothing else
at all, which is how a single edited JSON string once promoted a draft
archive to production (DD-47).
