...
the variable's exponent in the polynomial fit.
\end{reqbox}
...
Table~\ref{tab:unc-ops} is normative. All listed operations are linear in
the variable values, so propagation through the operation weights is
exact. No operation silently drops or silently carries an UncFrame.
The \code{smooth} and \code{diff} row remains provisional until OQ-18
is resolved during v0.2.0 implementation; an operation whose weight
rule is not yet frozen raises when uncertainty is present rather than
guessing. The M0 rows were validated against the implementation on
2026-07-21.
