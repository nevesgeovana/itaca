...
export format that can carry them, which is every export to a FILE.
Production mode never suppresses either record. The two in-memory
bridges, \code{db.to\_pandas()} and \code{db.to\_numpy()}, are declared
lossy escape hatches (REQ-71): a plain DataFrame and plain arrays have
nowhere to put the record, so the chain of custody ends there by design
