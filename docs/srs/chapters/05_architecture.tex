...
|   +-- compute.py             # compute() -- string equation derivation (AST)
...
+-- pproc/
|   +-- factory.py             # itc.processor() -- factory function
|   +-- protocol.py            # Processor typing.Protocol
|   +-- base.py                # EquationProcessor: what an .itceq defines
...
\code{itc.processor(name\_or\_path)} & Construct a processor by registered name or \code{.itceq} path (the factory is named apart from the \code{pproc} package it lives in; OQ-29).\\
...
\code{EquationProcessor}, etc.) implements the protocol without subclassing a
common base. AST operators (\code{Arith}, \code{Trig}, \code{Misc}) are
...
The decision is unchanged and its statement still holds; what widened
is the scope it is enforced over. REQ-82 was amended at the M1 Phase
B3b checkpoint to cover every package except \code{io/} and
\code{utils/}, so that a package added later is restricted by default
instead of inheriting nothing. Read REQ-82 for the normative scope;
DD-33 records why it is stated as an exception list.
\end{ddbox}
...
Specialized analyses are callable objects, invoked as \code{proc(db)}
on a constructed processor (the factory is \code{itc.processor}, named
apart from the \code{pproc} package; DD-34),
...
\addlinespace
\code{ProcessorError}   & \code{ProcessorNotFoundError}     & Unknown processor name passed to \code{itc.processor}\\
                        & \code{ProcessorValidationError}   & The frame cannot feed the processor: a required variable is missing, a \code{[constants]} name is a measured channel, or a built-in expression constant (\code{pi}, \code{e}) is a measured channel (REQ-45, DD-39, DD-42)\\
...
\addlinespace
\code{AxesError}        & \code{AxisNotFoundError}          & A referenced coordinate frame (\code{rotate} target, or a \code{declare\_vector} source) is not registered\\
                        & \code{VectorGroupError}           & A vector group cannot be resolved, is not a triplet, names an absent variable, or is re-declared\\
                        & \code{RotationMatrixError}        & An \code{Axis} definition is ambiguous or empty, its matrix is not a proper rotation (non-3$\times$3, non-orthogonal, or a reflection), the convention is unknown, or a frame name is duplicated\\
                        & \code{AxesError} (family, direct)  & \code{itc.concat} inputs express a vector group in different axis systems, or declare one group over different components (REQ-24, REQ-107). The family class is raised directly, since the condition is about disagreement ACROSS inputs rather than about one malformed axis, and no leaf describes it\\
\addlinespace
\code{DependencyError}  & \code{MissingDependencyError}     & Optional dependency (matplotlib, plotly, SMT, reportlab) absent\\
\addlinespace
\code{ITACAError} (direct) & \code{AccessorRegistrationError} & \code{itc.register\_accessor} name is not an identifier, or collides with an existing attribute or accessor (REQ-106). It has no family: registration errors of the extension point sit directly under \code{ITACAError}\\
\bottomrule
\end{tabularx}
\end{table}
