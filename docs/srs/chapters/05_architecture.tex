...
\code{AxesError}        & \code{AxisNotFoundError}          & A referenced coordinate frame (\code{rotate} target, or a \code{declare\_vector} source) is not registered\\
                        ...
                        & (the family class itself)          & \code{itc.concat} inputs express a vector group in different axis systems, or declare one group over different components (REQ-24, REQ-107). Raised as \code{AxesError} directly, since the condition is about disagreement ACROSS inputs rather than about one malformed axis, and no leaf describes it\\
\addlinespace
