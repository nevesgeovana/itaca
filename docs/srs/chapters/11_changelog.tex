...
\paragraph{Changed (Section~\ref{sec:itceq}; bears on REQ-41, REQ-45,
REQ-99).} The section rules now state WHEN a declared uncertainty is
applied, as two independent questions rather than one classification.
Applied before the first line when the VarFrame CARRIES the name, so every
line reading it propagates from the declared value; applied again after the
first line that WRITES it, so the frame does not ship a declaration as the
propagation of itself. A name answering yes to both is assigned twice. The
two are jointly exhaustive because \code{validate} refuses a declared name
that is neither (REQ-45).
