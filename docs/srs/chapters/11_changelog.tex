...
source frames and multi-frame data), draft. Design decision DD-26
(\code{scipy} as a dev-only direction-cosine test oracle, never a
runtime dependency) in Chapter~\ref{ch:architecture}. The axis
registry and vector-group declarations join the state hash
(REQ-103). The built-in wind and stability frames follow the AIAA
R-004A Etkin direction-cosine convention, SME-accepted at the B2
checkpoint.
