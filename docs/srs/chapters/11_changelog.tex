...
The CHK-1 release checkpoint added six public refusals to surfaces
...
The count reads six rather than five because the sixth was found after
this section was first written, by the independent review REV-004, and is
closed by \code{ITC-20260729-2255}. It is recorded as part of this
revision rather than as a later one because it is the same defect class
from the same checkpoint, and splitting it would suggest two amendments
where the work was one.

\paragraph{Changed (the six CHK-1 refusals).} REQ-01 states that one
...
(\code{R3-ITA-008}); it also states that a CSV row wider than its header
is refused with \code{DataError}, and that a row narrower than its header
is still filled with NaN, an asymmetry the requirement now gives its
reason for (\code{R3-ITA-009}). REQ-24 and REQ-107 both state that
