...
\code{0.2.0} ships three breaking changes. Nine carry the BREAKING
marker in \code{CHANGELOG.md}, measured 2026-07-29. The remedy is not a
corrected count inside the requirement: the enumeration is removed, and
REQ-92 now points at \code{CHANGELOG.md}, which the requirement's own
next paragraph already mandates as the place every breaking change is
marked with its migration. A second inventory in a normative document
goes stale independently of the one it duplicates, and this one did.

\paragraph{Changed (REQ-98 and REQ-26, one count in place of three).}
REQ-98 named its provisional operations in two places at two different
values, four and three, while the normative
...
done. The list is now given once, at FIVE:
...
The stated ground for \code{fill(method="polyfit")} joining the family
is that the polyfit path produces no weights at all, NOT that its
weights are data dependent: that earlier reasoning was measured false
(superposition departs by $1.3 \times 10^{-13}$) and, applied
consistently, would have drawn \code{interpolate} in as well.
...
Section~\ref{sec:dimension} calls it metadata, because
