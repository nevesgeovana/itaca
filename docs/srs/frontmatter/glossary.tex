...
\item[pproc]
The processor subpackage, \code{itaca.pproc}, reached as \code{itc.pproc}.
Holds the \code{Processor} protocol, the \code{.itceq} parser, the
registry, and the built-in processors. It is not the factory and not an
instance: those are \code{itc.processor} and a \emph{processor}
respectively (DD-34).
...
typing protocol, and is constructed by
\code{itc.processor(name\_or\_path)}.
