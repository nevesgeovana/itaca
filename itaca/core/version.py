...
AND IT DOES NOT REACH A TREE THAT WAS NEVER BUILT. The version file is
gitignored, so a fresh clone, a detached git worktree, or a CI leg
before ``pip install -e .`` does not have it, and resolution falls to
the metadata path with the ``sys.path`` scan intact. Measured: the same
plant that this reordering defeats on a built tree still wins on an
unbuilt one, reporting ``9.9.9.dev99``. It is not closed here because
the obvious closure is wrong: an editable install legitimately has its
``dist-info`` in site-packages while its code is in the source tree, so
a check that the distribution's location matches the imported package
would refuse the ordinary development case. The residual is pinned by
``tests/core/test_version_resolution.py``, which asserts BOTH meanings
against the tree under test rather than assuming which one applies.
"""
